...
yCBP(1,3) = 100;
for ii = 2:length(t)
    [tt, xx, ~, ~, ie] = ode45(@(t,y)guid_func_pp_rand_osc(t,y, vt, vu, theta_target,lambda,u), [ii-2 ii-1]*Ts, yCBP(ii-1,:),optPos);
    idxEnd = ii;
    yCBP(ii,:) = xx(end,:);
    if (~isempty(ie))
       tP(ii) = tt(end);
...
x_target = dyn_osc(tP(1:idxEnd),x_target_ini,vt*ones(idxEnd,1),theta_target*ones(length((1:idxEnd)),1));
